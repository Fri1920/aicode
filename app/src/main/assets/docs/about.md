...
*   **版本（点击检查更新）**：分组内一行，显示当前版本号 `v<versionName>`（通过系统 PackageManager 读取，即对外发布版本，与 git tag `v<versionName>` 一致）。点击该行手动检查更新：通过 GitHub API `https://api.github.com/repos/jieapi/aicode/releases` 查询发布列表，过滤出高于当前版本的版本（按所选更新通道），弹窗展示「发现新版本」并列出**从当前版本到最新版本的更新日志**；点击「前往下载」用浏览器打开 GitHub Releases 页面获取最新 APK。若已是最新 → 提示「已经是最新版本」；网络失败或解析失败 → 提示「检查失败」并给出错误信息。
